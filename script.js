...
async function saveWishToDb(data) {
  ...
  try {
    ...
    await window.db.collection("wishes").doc(id).set({
      ...data,
...
// Load a wish document by ID from Firestore
async function loadWishFromDb(docId) {
  if (!window.db) return null;
  try {
    const snap = await window.db.collection("wishes").doc(docId).get();
...
async function loadFromUrl() {
  ...
  if (!data) { dismissLoader(); return false; }
